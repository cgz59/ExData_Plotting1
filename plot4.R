...
par(mfrow=c(2,2))
with(sub_data, plot(DayTime,as.numeric(as.character(Global_active_power)),type = 'l', ylab="Global Active Power", xlab=""))
with(sub_data, plot(DayTime,as.numeric(as.character(Voltage)),type = 'l', ylab="Voltage", xlab="datetime"))
...
legend("topright",lty=1,legend=c("Sub_metering_1","Sub_metering_2","Sub_metering_3"),col=c("black","red","blue"),cex=0.75,bty = "n")
with(sub_data, plot(DayTime,as.numeric(as.character(Global_reactive_power)),type = 'l', ylab="Global_reactive_power", xlab="datetime"))
dev.off()
